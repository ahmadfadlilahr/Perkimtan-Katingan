        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold bg-gradient-to-r from-emerald-600 to-green-600 bg-clip-text text-transparent">
                    Edit {{ $types[$visiMisi->type] ?? ucfirst($visiMisi->type) }}
                </h2>
                <p class="mt-1 text-sm text-gray-600">
                    Perbarui {{ strtolower($types[$visiMisi->type] ?? $visiMisi->type) }} organisasi
                </p>
            </div>
        ...
        <div class="bg-white shadow-lg rounded-lg overflow-hidden">
            <div class="bg-gradient-to-r from-emerald-50 to-green-50 px-6 py-4 border-b border-gray-200">
                <h3 class="text-lg font-semibold text-gray-900 flex items-center">
                    ...
                    Form Edit {{ $types[$visiMisi->type] ?? ucfirst($visiMisi->type) }}
                </h3>
            </div>

            <!-- Form -->
            <form action="{{ route('dashboard.visi-misi.update', $visiMisi) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')

                <!-- Type (hidden) -->
                <input type="hidden" name="type" value="{{ $visiMisi->type }}">
                ...
                <div>
                    ...
                    <input type="text" id="title" name="title" value="{{ old('title', $visiMisi->title) }}" required
                           ...
                           placeholder="Masukkan judul {{ strtolower($types[$visiMisi->type] ?? $visiMisi->type) }}">
                    @error('title')
                ...
                <div>
                    ...
                    <textarea id="content" name="content" rows="6" required
                              ...
                              placeholder="Masukkan konten {{ strtolower($types[$visiMisi->type] ?? $visiMisi->type) }}">{{ old('content', $visiMisi->content) }}</textarea>
                    @error('content')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="order_position" name="order_position" value="{{ old('order_position', $visiMisi->order_position ?? 1) }}" min="1"
                ...
                <div>
                    ...
                    <div class="relative">
                        <label class="inline-flex items-center cursor-pointer">
                            ...
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $visiMisi->is_active) ? 'checked' : '' }}
                                   class="sr-only peer">
                ...
                <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-200">
                    ...
                    <button type="submit"
                        ...
                        Update {{ $types[$visiMisi->type] ?? ucfirst($visiMisi->type) }}
                    </button>
                </div>
            </form>
        </div>
        ...
        <div class="mt-8 bg-white shadow-lg rounded-lg overflow-hidden">
            ...
            <div class="p-6">
                <div id="preview-content" class="bg-gray-50 rounded-lg p-6 border-2 border-dashed border-gray-300">
                    <!-- Initial preview with existing data -->
                </div>
            </div>
        </div>
    </div>
